...
static void CG_InterpolatePlayerState( SyncPlayerState *playerState ) {
	...
	*playerState = *ps;
	...
	if( ps->weapon_state_time <= ops->weapon_state_time ) {
		...
		// assert( playerState->weapon_state_time == Max2( 0.0f, ops->weapon_state_time - cg.lerpfrac * dt ) );
	}
	else {
		s64 dt = cg.frame.serverTime - cg.oldFrame.serverTime;
		playerState->weapon_state_time = Max2( 0.0f, ops->weapon_state_time - cg.lerpfrac * dt );
	}
}
